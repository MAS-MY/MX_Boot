  ...
  else
  {
    if (dir == 0) /* Unicode to GBK */
    {
      if (src > 0x4DFF && src < 0x9FA6)
      {
        ...
        W25Q128_ReadData((uint32_t)unigbk, (uint8_t *)offset, 2); 
        c = ((uint16_t)unigbk[0] << 8) + (uint16_t)unigbk[1]; 
      }
      else 
      {
        c = 0xA1A1; // 如果是其它符号，都用NULL代替
      }
    }
    else if (dir == 1) /* GBK to Unicode */
    {
      ...
      W25Q128_ReadData((uint32_t)gbkuni, (uint8_t *)offset + 0x0D0000, 2); 
