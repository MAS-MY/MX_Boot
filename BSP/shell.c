...
static char *finsh_prompt_custom = "mmh >";
...
static rt_bool_t shell_handle_history(struct finsh_shell *shell)
{
    ...
    My_putstr("\033[2K\r");
#endif
    My_putstr(FINSH_PROMPT);
	My_putstr(g_shell.line);
    return RT_FALSE;
}
    ...
    while (1)
    {
        ...
        else if (ch == 0x7f || ch == 0x08)
        {
            ...
            if (g_shell.line_position > g_shell.line_curpos)
            {
                ...
                //My_putstr("\b%s  \b", &g_shell.line[g_shell.line_curpos]);
				putchar('\b');
				My_putstr(&g_shell.line[g_shell.line_curpos]);
				My_putstr("  \b");
